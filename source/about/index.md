---
title: ABOUT
date: 2026-02-21
layout: page
cover: /img/index.webp
---

{% cmd bat ~/about_me.json %}

```json ~/about_me.json
{
  "name": "Shiro",
  "location": ["North York, ON, CA"],
  "interests": ["Coding", "Music", "Gaming"],
  "contact": {
    "email": "brian.ltdt.soft@gmail.com"
  }
}
```


<!-- ### About Me

Hello 👋 I am a software engineer from Hong Kong 🇭🇰, currently actively looking for a job in Canada 🇨🇦, software engineer, machine learning engineer, full-stack developer, you name it.

In daily life, I game 🎮, listen to music 🎧, and torture myself with competitive programming 🧠. I like solving puzzles, there is something really satisfying about cracking a hard one.

This is my [LeetCode account](https://leetcode.com/u/seedalink60/).

This blog is just me sharing things I find interesting, mostly competitive programming thoughts 💡 and random ideas that are fun to write about. This is not my portfolio and I am not here to talk about my career, so sorry if that's what you were looking for.

I feel like one of the biggest reasons I started this blog is that I can't really find friends who are into the things I post about here. So if you read through and feel like we can hit the vibe, feel free to reach out 🙂. It would be crazy nice if you are also a Hongkonger who grinds LeetCode for fun.

You can reach me by [email](mailto:brian.ltdt.soft@gmail.com). -->

